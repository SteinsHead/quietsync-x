...
审核台先把真正要看的东西摆出来：词从哪里来、被分到了哪一类、哪些已经同步，以及哪些最好再看一眼。统计信息只占一小条，下面就是可以直接筛选和修改的词库。截图里的内容都是项目自带的演示数据，不是真实账号数据。

默认是清新的浅色界面，没有厚重侧栏，也没有为了“像后台”而摆进去的图表。右上角可以随时切到夜间模式；外观偏好只保存在浏览器本地，审核台和插件弹窗会保持一致。
...
Chrome Web Store 版本正在审核。在商店页面正式公开前，可以先从 GitHub 安装，整个过程大约一分钟：
